...
public class LineChartRenderer extends AbstractChartRenderer {
    ...
    public void draw(Canvas canvas) {
        ...
        for (Line line : data.getLines()) {
            if (line.hasLines()) {
                if (line.isCubic()) {//绘制平滑曲线
                    drawSmoothPath(drawCanvas, line);
                } else if (line.isSquare()) {//绘制平滑曲线
    ...
    private void drawSmoothPath(Canvas canvas, final Line line) {
        ...
        for (int valueIndex = 0; valueIndex < lineSize; ++valueIndex) {
            if (line.isContinuous()) {
                ...
                if (line.getValues().get(valueIndex).getY() > 0) {
                    if (valueIndex == 0 || needMoveTo) {
                        // Move to start point.
                        path.moveTo(currentPointX, currentPointY);
                    } else {
    ...
    private void drawPoints(Canvas canvas, Line line, int lineIndex, int mode) {
        ...
        int valueIndex = 0;
        for (PointValue pointValue : line.getValues()) {
            if (line.isContinuous() || pointValue.getY() > 0) {
                int pointRadius = ChartUtils.dp2px(density, line.getPointRadius());
                final float rawX = computator.computeRawX(pointValue.getX());
                final float rawY = computator.computeRawY(pointValue.getY());
